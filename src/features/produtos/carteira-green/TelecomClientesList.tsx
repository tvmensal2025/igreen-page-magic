...
export function TelecomClientesList({ consultantId }: { consultantId: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((c) =>
      `${c.nome || ""} ${c.cidade || ""} ${c.uf || ""} ${c.numero || ""} ${c.licenciado || ""}`
        .toLowerCase()
        .includes(s),
    );
  }, [data, q]);
  ...
  const mrr = data.filter((c) => (c.status || "").toLowerCase().includes("ativ")).reduce((s, c) => s + Number(c.fatura_valor || 0), 0);

  return (
    <section className="rounded-xl border border-border/60 bg-card">
      <header className="p-4 border-b border-border/60 space-y-2">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h3 className="text-sm font-semibold">Clientes Telecom · {data.length}</h3>
          <span className="text-[11px] text-muted-foreground">
            MRR ativos: <strong>{BRL(mrr)}</strong>
          </span>
        </div>
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar cliente, número, cidade…" className="pl-9" />
        </div>
      </header>
