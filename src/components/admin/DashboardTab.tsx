  ...
  return (
    <div ref={dashboardRef} className="space-y-6" data-tour="dashboard">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-[12px] min-w-0">
            <div className="flex flex-wrap items-center gap-2 text-amber-900 min-w-0">
              {analyticsMissing ? (
                ...
                <>
                  <span className="break-words">No banco: <b>{walletTotal}</b> clientes iGreen · Exibidos: <b>{shown}</b>{selectedLicenciado !== "all" && <> · Filtro: <b>{selectedLicenciado}</b></>}</span>
                </>
              ) : (
      ...
      <div className="grid grid-cols-1 min-[380px]:grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-4 min-w-0" data-tour="painel-kpis">
        <StatCard icon={<Users className="w-5 h-5" />} label="analise o envio em massa, e a estrutura que criamos, todos envio em massa todos os usuarios estao bloqueado, o sistema continua enviando msg e esta proibido a essas pessas" value={filteredMetrics?.totalCustomers ?? 0} color="primary" />
