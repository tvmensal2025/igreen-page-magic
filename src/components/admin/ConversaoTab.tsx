...
import {
  Loader2, RefreshCw, Flame, Cloud, Snowflake, Skull,
...
interface ConversaoTabProps {
  consultantId: string;
}

export function ConversaoTab({ consultantId }: ConversaoTabProps) {
        ...
        {(Object.keys(TEMP_META) as Temp[]).map(t => {
          ...
          return (
            ...
            >
              ...
              <div className="text-2xl font-semibold mt-0.5 text-foreground">{counts[t]}</div>
            </button>
          );
        })}
      </div>
