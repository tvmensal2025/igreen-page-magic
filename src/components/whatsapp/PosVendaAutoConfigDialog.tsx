    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        <DialogHeader className="shrink-0 px-4 sm:px-6 pt-5 pb-3 pr-12 border-b text-left space-y-2">
          <DialogTitle className="text-base sm:text-lg">Mensagens automáticas do Pós-Venda</DialogTitle>
          <DialogDescription className="text-xs sm:text-sm leading-relaxed">
            O que o cliente recebe em cada coluna. Usa o WhatsApp do consultor (ou o compartilhado).
          </DialogDescription>
        </DialogHeader>
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {stages.map((s) => (
                <div
                  key={s.id}
                  className="flex flex-col gap-3 sm:flex-row sm:items-center p-3 rounded-lg border border-border/60 bg-card/50"
                >
