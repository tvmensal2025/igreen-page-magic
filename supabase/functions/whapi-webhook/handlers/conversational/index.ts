...
export async function runConversationalFlow(ctx: BotContext): Promise<BotResult> {
  ...
  if (qaHit) {
    ...
    if (qaReturnStep.id !== currentStep.id) {
      console.log(`[conversational] FAQ return ${currentStep.step_key} → ${qaReturnStep.step_key}`);
    ...
    if (anyEmitted && !qaReemitted && audioEmitted) {
      ...
      if (close) {
        try {
          ...
          console.warn("[conversational] close pós-áudio falhou:", (e as Error)?.message || e);
        }
      }
    }
    ...
    try {
      ...
      if (answerText && orch.confidence >= 0.55) {
        ...
        if (orchReturnStep.id !== currentStep.id) {
          console.log(`[conversational-orch] return ${currentStep.step_key} → ${orchReturnStep.step_key}`);
