...
// Regressão do bug 2026-05-30 (número 11971254913): o handler lia só
// `success_goto_step_id` e ignorava `goto_step_id`, caindo no CHAIN amplo
// que despachava d_como_funciona + d_resultado juntos.
// ─────────────────────────────────────────────────────────────────────
...
Deno.test("resolvePostBillNextStepId: goto_step_id é ignorado quando mode != goto", () => {
  // fallback.mode=repeat com goto_step_id residual NÃO deve forçar destino.
  assertEquals(
    resolvePostBillNextStepId({ mode: "repeat", goto_step_id: "x" }),
    null,
  );
});
