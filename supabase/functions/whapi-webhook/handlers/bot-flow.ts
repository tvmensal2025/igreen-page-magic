...
export async function runBotFlow(ctx: BotContext): Promise<BotResult> {
          ...
          if (ai.text && ai.confidence >= 0.55) {
            ...
            if ((_fbVarAi === "D" || _fbVarAi === "M") && !baseUpdates.bot_paused) {
              try {
                ...
                console.warn("[ai-global] button re-emission failed:", (e as Error).message);
              }
            }

            return { reply: "", updates: { ...baseUpdates, __inline_sent: true } as any };
          } else {
  ...
  if (
    ...
    // Push-name plausível (Viviane) → não re-pede nome; saudação continua sem "Oi Nome".
    ...
    // Off-topic ("Isso é um teste") com nome push/confiável → pede valor, não trava no nome.
      ...
      if (flow?.id) {
        ...
        if (stepRow) {
          ...
          if (stype === "capture_conta") {
            // Guarda o UUID do capture_conta (sim vs cadastro) antes de virar
            // "aguardando_conta" — o pós-OCR usa isso pra não mandar Ativar→resultado.
          ...
          // bot pular o documento sem processar a foto.
            ...
            if (nextCustom) {
              ...
              if (ntype === "capture_conta") {
                ...
                _isCapture = true;
                if (current?.id) {
                  // Persiste qual conta (sim/cadastro) pediu a foto
                  (customer as any).previous_conversation_step = String(current.id);
                }
              }
  ...
  switch (step) {
      ...
      if (resp === "sim_conta" || resp === "sim" || resp === "s" || resp === "1" || resp === "ok" || resp === "correto" || resp === "✅") {
        ...
        try {
          ...
          if (_flowRowSuccess?.id) {
            ...
            console.log(`[post-confirm-conta] capture escolhido=${(_captureStep as any)?.step_key || "null"} (preferred=${(customer as any).previous_conversation_step || "—"})`);
        ...
        if (_hasExplicitSuccessGoto && nextCustom && nextCustom.step_type === "message") {
          ...
          try {
            const _flowRow3 = await resolveFlowId(supabase, customer.consultant_id, (customer as any)?.flow_variant || "A");
            if (_flowRow3?.id) {
          ...
          try {
            const _flowRow2 = await resolveFlowId(supabase, customer.consultant_id, (customer as any)?.flow_variant || "A");
            if (_flowRow2?.id) {
              const { data: _allSteps } = await supabase
                .from("bot_flow_steps")
    ...
    case "aguardando_assinatura": {
      ...
      // Produção: NÃO concluir facial só por texto ("PRONTO"). O watchdog/portal
      // confirma o status oficial. Texto livre só em sandbox (teste E2E).
  ...
  if (updates.conversation_step === "finalizando") {
    ...
    if (!validation.valid) {
      ...
      // (send fica após dispatchPortalWorker abaixo)
      }
