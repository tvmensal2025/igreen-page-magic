...
function extractCopy(creative: any): {
  ...
  if (link) {
    ...
    thumb_url = link.picture || link.image_url ||
      link.child_attachments?.[0]?.picture || null;
    image_hash = link.image_hash || link.child_attachments?.[0]?.image_hash ||
      null;
  } else if (video) {
  ...
  if (afs) {
    ...
    if (!thumb_url) {
      thumb_url = afs.images?.[0]?.url || afs.videos?.[0]?.thumbnail_url ||
        null;
    }
    if (!video_id && afs.videos?.[0]?.video_id) {
      video_id = afs.videos[0].video_id;
    }
  ...
  if (!thumb_url) {
    thumb_url = creative.thumbnail_url || creative.image_url ||
      creative.image_urls?.[0] || null;
  }
...
async function resolveImageHashThumb(
  ...
  try {
    const acc = adAccountId.startsWith("act_")
      ? adAccountId
      : `act_${adAccountId}`;
    const url =
      `${FB_GRAPH}/${acc}/adimages?hashes=${
    ...
    const pic = fromMap?.url || fromMap?.permalink_url || fromList?.url ||
      fromList?.permalink_url || null;
    cache.set(hash, pic);
    return pic;
  } catch (e) {
    console.warn(
      "[fb-sync-creatives] image_hash thumb fail",
      hash,
      (e as Error).message,
    );
    cache.set(hash, null);
...
async function resolveVideoThumb(
  videoId: string,
  token: string,
  cache: Map<string, string | null>,
): Promise<string | null> {
  ...
  try {
    ...
    console.warn(
      "[fb-sync-creatives] video thumb fail",
      videoId,
      (e as Error).message,
    );
...
async function getCreativeCopy(
  creativeId: string,
  token: string,
  cache: Map<string, any>,
): Promise<ReturnType<typeof extractCopy>> {
  if (cache.has(creativeId)) return cache.get(creativeId);
  try {
    const url =
      `${FB_GRAPH}/${creativeId}?fields=name,title,body,thumbnail_url,image_url,object_story_spec,asset_feed_spec&access_token=${token}`;
    const json = await fbFetch(url);
    ...
    console.warn(
      "[fb-sync-creatives] copy fetch fail",
      creativeId,
      (e as Error).message,
    );
    const empty = {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      if (body && typeof body.consultant_id === "string") {
    ...
    if (!campaigns?.length) {
      return new Response(JSON.stringify({ processed: 0, ads_synced: 0 }), {
    ...
    for (const c of campaigns) {
      try {
        if (!tokenCache[c.consultant_id]) {
          ...
          if (!conn) {
            errors.push({ campaign_id: c.id, error: "sem conexão Meta" });
            continue;
          }
          tokenCache[c.consultant_id] = {
            token: conn.token,
        ...
        const adsUrl =
          `${FB_GRAPH}/${c.fb_campaign_id}/ads?fields=id,name,status,creative{id,thumbnail_url,image_url}&limit=100&access_token=${token}`;
        ...
        const since = new Date(Date.now() - 14 * 86400_000).toISOString().slice(
          0,
          10,
        );
        const until = new Date().toISOString().slice(0, 10);
        const insightsUrl =
          `${FB_GRAPH}/${c.fb_campaign_id}/insights?level=ad&fields=ad_id,impressions,clicks,spend,actions&time_range={"since":"${since}","until":"${until}"}&access_token=${token}`;
        const insJson = await fbFetch(insightsUrl);
        const insightsByAd = new Map<string, any>();
        for (const row of insJson?.data || []) {
          insightsByAd.set(String(row.ad_id), row);
        }
        ...
        for (const ad of ads) {
          ...
          const copy = await getCreativeCopy(
            ad.creative.id,
            token,
            creativeCache,
          );
          ...
          const spend_cents = ins
            ? Math.round(parseFloat(ins.spend || "0") * 100)
            : 0;
          ...
          if (!thumb && copy.image_hash && ad_account_id) {
            thumb = await resolveImageHashThumb(
              ad_account_id,
              copy.image_hash,
              token,
              imageHashCache,
