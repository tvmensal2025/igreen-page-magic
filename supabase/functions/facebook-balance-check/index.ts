...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Este handler só executa pausa por saldo zerado ou teto atingido — ação
    // PROTETIVA. Não existe gate de automação aqui: travá-lo deixaria a Meta
    // gastando depois da carteira zerar e do teto reservado ser estourado.
    // Ver a assimetria em `_shared/ad-automation-policy.ts`.
    ...
    for (const c of pending || []) {
      try {
        if (c.status === "active" && c.fb_campaign_id) {
          ...
          if (conn?.token) {
            await fbFetch(
              `/${c.fb_campaign_id}?status=PAUSED&access_token=${conn.token}`,
              { method: "POST" },
            );
          }
        }
        await admin.from("facebook_campaigns").update({
          ...
          pause_pending: false,
          rejection_reason:
            "Auto-pausada: saldo da carteira zerou — recarregue para reativar",
        }).eq("id", c.id);
        ...
        try {
          ...
          console.error(
            "[fb-balance-check] rodizio notify:",
            (e as Error).message,
          );
        }
      } catch (e) {
      ...
      try {
        ...
        if (spendCents >= Math.floor(cap * 0.95)) {
          ...
          await admin.from("facebook_campaigns").update({
            status: "paused",
            pause_pending: false,
            rejection_reason: `Auto-pausada: gastou R$ ${
              (spendCents / 100).toFixed(2)
            } do teto reservado de R$ ${
              (cap / 100).toFixed(2)
    ...
    return json(
      {
        ...
        skipped,
        errors,
        checked: (pending?.length || 0) + (capped?.length || 0),
      },
      200,
      corsHeaders,
    );
