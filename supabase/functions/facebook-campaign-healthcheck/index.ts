...
async function isCronCaller(
  ...
  if (
    ...
    timingSafeEqual(headerService, serviceSecret)
  ) {
    return true;
  }
  ...
  if (
    ...
    timingSafeEqual(headerInternal, expectedInternal)
  ) {
    return true;
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: stuck } = await admin
      ...
      .select(
        "id, consultant_id, fb_campaign_id, fb_adset_ids, fb_ad_ids, status, created_at, rejection_reason",
      )
    ...
    const consultantIds = Array.from(
      new Set((stuck || []).map((campaign) => campaign.consultant_id)),
    );
    ...
    for (const c of stuck || []) {
      // Recovery do cron = REATIVAR campanha pausada, ou seja, voltar a gastar.
      // Ação `activate` na policy central: expansiva, exige modo explícito.
      // O clique manual do consultor segue por outro caminho, mais abaixo.
      if (
        ...
        results.push({
          id: c.id,
          activated: false,
          reason: "ads_automation_disabled",
        });
        continue;
      }
      ...
      if (c.status === "paused" && isConsultantLocked(c.rejection_reason)) {
        results.push({
          id: c.id,
          activated: false,
          reason: "skipped_manual_pause",
        });
        continue;
      }
      // Paused sem motivo recuperável (ex.: saldo, teto, ou pause limpa): não reativa.
      if (
        c.status === "paused" && !isRecoverableAutoPause(c.rejection_reason)
      ) {
        results.push({
          id: c.id,
          activated: false,
          reason: "skipped_non_recoverable_pause",
        });
        continue;
      }
    ...
    return j(req, { scanned: results.length, results });
  } catch (e) {
...
async function reactivateOne(
  ...
  const { data: c } = await admin
    .from("facebook_campaigns")
    .select(
      "fb_campaign_id, fb_adset_ids, fb_ad_ids, status, rejection_reason, consultant_id, daily_budget_cents, duration_days, end_time_utc",
    )
    .eq("id", campaignDbId)
    .maybeSingle();
  if (!c?.fb_campaign_id) {
    return { activated: false, reason: "Campanha sem ID Meta" };
  }
  ...
  const remainingDays = c.end_time_utc
    ? Math.max(
      1,
      Math.ceil((new Date(c.end_time_utc).getTime() - Date.now()) / 86400_000),
    )
    : null;
  ...
  if (!activationBudget.ok) {
    return {
      activated: false,
      reason: activationBudget.error || "insufficient_wallet",
    };
  }

  const rodizio = await validateRodizioActivation(
    admin,
    campaignDbId,
    c.consultant_id,
  );
  ...
  if (!conn?.token) {
    return {
      activated: false,
      reason: "Sem token Meta (plataforma desconectada)",
    };
  }
  ...
  try {
    ...
    const [campaignState, ...children] = await Promise.all([
      fbFetch(
        `/${c.fb_campaign_id}?fields=effective_status,configured_status,issues_info&access_token=${
    ...
    if (resolved.localStatus === "pending_review") {
      ...
      return {
        ...
        reason:
          "Ativação enviada. A Meta ainda analisa os anúncios (IN_PROCESS) — isso é normal e costuma liberar em minutos.",
      };
    }
    ...
    if (resolved.localStatus !== "active") {
      ...
      return {
        ...
        pending_review: true,
        effective_statuses: resolved.objectStatuses,
        reason: `Ativação enviada. Status atual na Meta: ${
          resolved.objectStatuses.join(", ")
        }.`,
      };
    }
    ...
    if (
      ...
      reason =
        "SESSION_INVALIDATED: O token do Facebook foi invalidado (senha alterada ou sessão encerrada por segurança). Reconecte a conta Facebook no painel. | " +
        raw;
    }
