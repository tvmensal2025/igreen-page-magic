-- Portal parceiro: devolve dados do consultor para rodapé do banner (download).
...
COMMENT ON FUNCTION public.get_partner_banner_portal(text) IS
  'Portal publico /p/{token}: cycle_leads mascarados + consultant (name/igreen_id/phone) para download de banner.';
