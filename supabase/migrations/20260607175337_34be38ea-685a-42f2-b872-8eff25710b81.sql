...
-- 1) Quarentena pós-reset (telefone tratado como "número novo" por 60s)
CREATE TABLE IF NOT EXISTS public.phone_reset_quarantine (
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.phone_reset_quarantine TO service_role;
-- intencionalmente sem grants para anon/authenticated: só edge functions (service_role) leem/gravam

ALTER TABLE public.phone_reset_quarantine ENABLE ROW LEVEL SECURITY;

CREATE POLICY "service role full access"
  ON public.phone_reset_quarantine
  FOR ALL
  TO service_role
  USING (true) WITH CHECK (true);

CREATE INDEX IF NOT EXISTS idx_phone_reset_quarantine_until
  ON public.phone_reset_quarantine (quarantine_until DESC);

-- 2) admin_hard_reset_phone v2: apaga resíduos novos + grava quarentena
...
BEGIN
  ...
  -- 🆕 Novos resíduos descobertos na auditoria
  BEGIN
  ...
  EXCEPTION WHEN undefined_table THEN v_deleted := v_deleted || jsonb_build_object('bot_flow_rule_fires', 0);
  END;
  ...
  EXCEPTION WHEN undefined_table THEN v_deleted := v_deleted || jsonb_build_object('campaign_match_log', 0);
  END;

  BEGIN
    DELETE FROM capture_achievements WHERE customer_id = ANY(v_ids);
    GET DIAGNOSTICS v_count = ROW_COUNT; v_deleted := v_deleted || jsonb_build_object('capture_achievements', v_count);
  EXCEPTION WHEN undefined_table THEN v_deleted := v_deleted || jsonb_build_object('capture_achievements', 0);
  END;
  ...
  EXCEPTION WHEN undefined_table THEN v_deleted := v_deleted || jsonb_build_object('referral_bonuses', 0);
  END;
  ...
  -- 🆕 Quarentena pós-reset: webhooks ignoram callbacks/recibos atrasados por 60s
  ...
  -- Cobre também a variante local (sem 55) caso o webhook normalize diferente
