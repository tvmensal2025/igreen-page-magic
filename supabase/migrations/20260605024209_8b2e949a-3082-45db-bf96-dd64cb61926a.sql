...
BEGIN
  ...
  BEGIN
    PERFORM public.log_admin_action(
      ...
      'phone',
      v_norm,
      jsonb_build_object('deleted', v_deleted, 'variants', v_variants, 'customer_ids', v_ids)
    );
  EXCEPTION WHEN OTHERS THEN
    RAISE LOG 'admin_hard_reset_phone audit log skipped: %', SQLERRM;
  END;

  RAISE LOG 'admin_hard_reset_phone done phone=% deleted=% customer_ids=%', v_norm, v_deleted, v_ids;
...
REVOKE EXECUTE ON FUNCTION public.admin_hard_reset_phone(text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.admin_hard_reset_phone(text) FROM anon;
GRANT EXECUTE ON FUNCTION public.admin_hard_reset_phone(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_hard_reset_phone(text) TO service_role;