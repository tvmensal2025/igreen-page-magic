-- Portal parceiro: cycle_leads só elegíveis + fila diária + campos pós-venda.
...
BEGIN
  ...
  -- Exige presença em lead_cadence_state OU fila diária de hoje.
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    'id', c.id,
    'name', c.name,
...
COMMENT ON FUNCTION public.get_partner_banner_portal(text) IS
  'Portal /p/{token}: cycle_leads só elegíveis (nome/fone/stage/fila). Token secreto.';
