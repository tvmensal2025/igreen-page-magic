-- Portal parceiro: handoff na pizza SEM depender de deploy do front.
--
-- O RPC já devolvia Robinho em cycle_leads, mas o bundle antigo do site
-- classificava stage=PAUSED + paused_reason=handoff_humano como FORA da pizza
-- (isPausedGroupA não conhecia handoff). Corrigimos o TS no commit 74aad7548,
-- porém o front em produção não redeploya sozinho (só edges no Actions).
--
-- Compat: para handoff, o JSON do RPC envia stage=AI_QUALIFYING (fatia Ativo)
-- + portal_group/portal_slice para o bundle novo. O antigo passa a contar 1
-- em "Em conversa (A)" na hora do refresh.
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    ...
    'stage', CASE
      WHEN lower(coalesce(lcs.paused_reason, '')) = 'handoff_humano'
        OR coalesce(c.bot_paused_reason, '') LIKE 'humano_assumiu%'
      THEN 'AI_QUALIFYING'
      ELSE lcs.stage::text
    END,
    'stage_actual', lcs.stage::text,
    'portal_group', CASE
      WHEN lower(coalesce(lcs.paused_reason, '')) = 'handoff_humano'
        OR coalesce(c.bot_paused_reason, '') LIKE 'humano_assumiu%'
      THEN 'A'
      WHEN upper(coalesce(q.queue, '')) = 'A' THEN 'A'
      WHEN upper(coalesce(q.queue, '')) = 'B' THEN 'B'
      WHEN upper(coalesce(q.queue, '')) = 'C' THEN 'C'
      WHEN lcs.stage::text IN ('NEW','GREETED','AI_QUALIFYING','A_NUDGE','A_SMS','A_CALL','A_CALL_RETRY') THEN 'A'
      WHEN lcs.stage::text LIKE 'COLD_%' OR lcs.stage::text LIKE 'SMS_%' OR lcs.stage::text LIKE 'CALL_%' THEN 'B'
      WHEN lcs.stage::text LIKE 'RECALL_%' OR lcs.stage::text IN ('CLOSE_LOST','RETARGET_META','RETARGET_ADS_15D') THEN 'C'
      ELSE NULL
    END,
    'portal_slice', CASE
      WHEN lower(coalesce(lcs.paused_reason, '')) = 'handoff_humano'
        OR coalesce(c.bot_paused_reason, '') LIKE 'humano_assumiu%'
      THEN 'flow'
      WHEN upper(coalesce(q.queue, '')) = 'A' THEN q.step
      WHEN upper(coalesce(q.queue, '')) = 'B' THEN q.step
      WHEN upper(coalesce(q.queue, '')) = 'C' THEN q.step
      WHEN lcs.stage::text = 'NEW' THEN 'ask_name'
      WHEN lcs.stage::text = 'GREETED' THEN 'wait'
      WHEN lcs.stage::text = 'AI_QUALIFYING' THEN 'flow'
      WHEN lcs.stage::text = 'A_NUDGE' THEN 'nudge'
      WHEN lcs.stage::text = 'A_SMS' THEN 'sms'
      WHEN lcs.stage::text = 'A_CALL' THEN 'call1'
      WHEN lcs.stage::text = 'A_CALL_RETRY' THEN 'retry'
      ELSE NULL
    END,
    'paused_reason', lcs.paused_reason, 'next_action_at', lcs.next_action_at,
