-- Portal do parceiro: dizer ONDE o lead está e parar de inflar leitura.
--
-- PROBLEMA REAL (José, 2026-08-05)
-- -------------------------------
-- O portal mostrava "LEADS 2" e, logo abaixo, "0 pessoas no ciclo". Os dois
-- números estavam certos e mesmo assim o parceiro concluiu que o sistema
-- tinha perdido a gente dele. Os 2 leads estavam fora da pizza por motivos
-- COMPLETAMENTE diferentes: um fechou o cadastro, o outro estava em
-- atendimento humano (o consultor respondeu por áudio pelo celular, o webhook
-- marcou `humano_assumiu` e a cadência parou). Zero seco não é resposta.
--
-- E as "leituras": 9 para 2 conversas. Parte do número era diagnóstico
-- (`qr-redirect?json=1`) e a mesma abertura contada duas vezes com 1 segundo
-- de diferença. O `qr-redirect` passou a filtrar isso na origem; aqui a
-- contagem também ignora repetição em janela curta, para o histórico já
-- gravado não continuar mentindo.
...
BEGIN
  ...
  -- Leitura = uma pessoa abrindo o link. Duas batidas no mesmo alvo dentro de
  -- 15s são a mesma abertura (prefetch, duplo toque, preview de link).
  ...
  -- Todo lead que existe e não aparece na pizza precisa de um motivo legível.
    ...
    SELECT
      CASE
        ...
        WHEN lower(coalesce(lcs.paused_reason, '')) = 'handoff_humano'
          OR coalesce(c.bot_paused_reason, '') = 'humano_assumiu' THEN 'atendimento_humano'
