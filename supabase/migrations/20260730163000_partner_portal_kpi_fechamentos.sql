-- Portal parceiro: KPI Fechamentos (= aprovados iGreen do parceiro).
...
BEGIN
  ...
  -- Mesmo critério de get_referral_partner_analytics.aprovados
  SELECT count(*)::bigint
  INTO v_fechamentos
  FROM customers c
  WHERE c.referral_partner_id = v_partner.id
...
COMMENT ON FUNCTION public.get_partner_banner_portal(text) IS
  'Portal publico /p/{token}: KPIs (fechamentos=aprovados), cycle_leads mascarados, consultant para download.';
