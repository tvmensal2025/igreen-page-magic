...
BEGIN
  ...
  SELECT array_agg(DISTINCT id) INTO v_ids FROM (
    ...
    SELECT customer_id AS id FROM bot_step_transitions
     WHERE regexp_replace(coalesce(phone,''), '\D', '', 'g') = ANY(v_variants)
  ...
  -- Captura consultores afetados ANTES de apagar customers,
  -- para depois zerar o contador diário de envios das instâncias deles.
  SELECT array_agg(DISTINCT consultant_id)
    INTO v_consultant_ids
    FROM customers
   WHERE id = ANY(v_ids) AND consultant_id IS NOT NULL;
  ...
  -- 🆕 Zera o contador diário de envios das instâncias dos consultores
  -- afetados, para liberar áudio/vídeo/texto após "Resetar telefone".
  -- Restrito a hoje (UTC) e às instâncias dos consultores envolvidos.
  ...
  BEGIN
    PERFORM public.log_admin_action(
      ...
      'phone',
      v_norm,
      jsonb_build_object('deleted', v_deleted, 'variants', v_variants, 'customer_ids', v_ids, 'consultant_ids', v_consultant_ids)
    );
  EXCEPTION WHEN OTHERS THEN
    RAISE LOG 'admin_hard_reset_phone audit log skipped: %', SQLERRM;
  END;

  RAISE LOG 'admin_hard_reset_phone done phone=% deleted=% customer_ids=% consultant_ids=%', v_norm, v_deleted, v_ids, v_consultant_ids;
