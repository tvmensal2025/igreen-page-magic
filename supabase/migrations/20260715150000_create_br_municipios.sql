-- Catálogo geográfico IBGE (municípios do Brasil).
-- Separado de fb_city_cache (chaves Meta Ads) e de CITY_HINT (cobertura comercial).
...
CREATE INDEX IF NOT EXISTS idx_br_municipios_name_normalized
...
COMMENT ON TABLE public.br_municipios IS
  'Catálogo IBGE de municípios BR (~5570). Autocomplete geográfico. Não misturar com fb_city_cache (Meta) nem CITY_HINT (cobertura).';
