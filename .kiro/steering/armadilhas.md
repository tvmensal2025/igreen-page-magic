...
42. **Cliente carteira recebe msg de lead novo** → sync grava colisão `5511…_igreenCode` e deixa lead sombra no número limpo; webhook buscava só exact → Grupo A no lead. Corrigir: `_shared/inbound-customer-resolve.ts` (prioriza `igreen_sync`), absorver sombra no sync, não notificar carteira como novo lead. Idempotência PV: `customer_auto_message_log` UNIQUE `(customer_id, stage_key)` — `sent` nunca reenvia.
