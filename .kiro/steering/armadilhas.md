...
46. **Envio a lead sem `assertCanContact`** → `finalize-capture`, `recover-stuck-otp` (loop lead), `worker-callback` precisam do gate DNC. Portal/OTP: use **só** `assertCanContact` (não `assertBotOutboundAllowed` — `bot_global` não pode cortar OTP/facial). Pós-venda: `isPausedByPhone` já cobre DNC; não forçar `assertBotOutboundAllowed` (ignora `bot_global` por desenho). Teste: `src/test/outbound-dnc-coverage.test.ts`.
47. **`recon-igreen-worker` sem auth** → auth híbrida: `assertCronAuth` **ou** admin JWT (`resolveCaller.isAdmin`). `config.toml` com `verify_jwt=false`. Sem isso, anon/JWT comum podia consumir fila + IA.
