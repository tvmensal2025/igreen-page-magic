...
46. **Envio a lead sem `assertCanContact`** → `finalize-capture`, `recover-stuck-otp` (loop lead), `worker-callback`, **`voice-dialer-webhook` SMS fallback NA** precisam do gate DNC. Portal/OTP: use **só** `assertCanContact` (não `assertBotOutboundAllowed` — `bot_global` não pode cortar OTP/facial). Pós-venda: `isPausedByPhone` já cobre DNC; não forçar `assertBotOutboundAllowed` (ignora `bot_global` por desenho). Teste: `src/test/outbound-dnc-coverage.test.ts`.
...
49. **`finalize-capture` IDOR** → não basta “tem JWT”. Exigir `resolveCaller` + `assertOwnership({ customerId })` (consultor dono ou admin; `x-service-secret` só interno). Sem posse, qualquer autenticado disparava Portal 2 no lead alheio.
