...
40. **Subconta sync “não puxa telefone” / cliente fica `sem_celular_*`** → Conta principal mascara `celular` da rede; unique `(consultant_id, igreen_code)` trava 1 linha. **UPDATE por igreen_code** (não upsert só por phone); enrich **sem** filtrar `igreen_account_id`; `enrich_only` reprocessa placeholder pelo `registered_by`. Ver `#igreen-sync-oficial`.
41. **Cliente carteira recebe msg de lead novo** → sync grava colisão `5511…_igreenCode` e deixa lead sombra no número limpo; webhook buscava só exact → Grupo A no lead. Corrigir: `_shared/inbound-customer-resolve.ts` (prioriza `igreen_sync`), absorver sombra no sync, não notificar carteira como novo lead. Idempotência PV: `customer_auto_message_log` UNIQUE `(customer_id, stage_key)` — `sent` nunca reenvia.
