...
- Página do parceiro: `PartnerBannerPortalPage` (`/p/{portal_token}`) + RPC `get_partner_banner_portal`.
  ...
  - **KPIs canônicos (não reabrir):**
    ...
    - `outside_cycle` = lead que existe e **não** está na pizza, por motivo: `fechado` · `cadastro_em_analise` · `bloqueado` · `cliente_carteira` · `sem_cadencia`. **Handoff humano entra na pizza** (fatia Ativo / Em conversa), com aviso “falando com o consultor” — pausado ≠ sumiu (armadilha #65).
